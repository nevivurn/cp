...
for atk in range(4, 10000):
    changed = []
    for u in units:
        if u.race == 'E':
            u = Unit(u.id, u.hp, atk, u.race, u.x, u.y)
        changed.append(u)

    cnt, changed = combat(changed)
    if any(u.race == 'E' and u.hp <= 0 for u in changed):
        continue

    remhp = sum(u.hp for u in changed if u.hp > 0)
    print('answer:', remhp * cnt)
    break
